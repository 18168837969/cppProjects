 ...
 */
  ...
  *  \internal
  */
QMap<QString, QPushButton*> QtMaterialFlatButtonPrivate::mutexChecked;
...
void QtMaterialFlatButton::RegisterMutex(const QString& className) {
	...
	connect(this, &QPushButton::clicked, this, [&](bool ischeked) {
		Q_D(QtMaterialFlatButton);
		QString className = this->property("className").toString();
		QPushButton* _lastActiveBtn = d->mutexChecked[className];
		...
		else if (button && button->isChecked())
		{
			...
			d->mutexChecked[className] = button;
		}
		});
}
...
void QtMaterialFlatButton::setCheckedColor(const QColor& color) {
	Q_D(QtMaterialFlatButton);
...
QColor QtMaterialFlatButton::checkedColor() const {
	Q_D(const QtMaterialFlatButton);
...
void QtMaterialFlatButton::paintBackground(QPainter* painter)
{
	...
	//default
	if (isCheckable() && checkedProgress > 0&&d->checkedColor.name()=="#000000") {
	...
	else if(isCheckable() && this->isChecked() && d->checkedColor.name() != "#000000")
	{	
...
void QtMaterialFlatButton::paintForeground(QPainter* painter)
{
	...
	if (isEnabled()) {
		...
		if (isCheckable() && progress > 0&&d->checkedColor.name()=="#000000") {
